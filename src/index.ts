...
export function getRunnerVersion(runnerBinaryPath?: string): string {
  ...
  if (!fs.existsSync(holochainRunnerBinaryPath)) {
    console.error("holochain-runner binary not found at path: " + holochainRunnerBinaryPath)
  ...
  if (holochainHandle.error) {
    console.error("Calling holochain-runner failed: " + holochainHandle.error.message)
